...
When `identities` are configured, Person Spotlight also lists logical subjects marked **all
accounts**, and Multi-Person lists saved groups marked **ANY** or **ALL**. The other date-based
presets show an optional **Cross-account people** filter, so Monthly Highlights can become a Kids
highlight without creating a separate Immich album. ANY means at least one configured subject is
in the asset; ALL means every configured subject is in the same asset.

Person Spotlight and Multi-Person also offer an **Annual birthday/anniversary story** checkbox for
configured logical selections. It combines previous occurrences of the configured date with the
year ending on the selected occurrence, then keeps the resulting photos and video clips in
chronological order. The initial target is five minutes; Step 2 can set any other target.
